...
async def query_document(
    ...
    try:
        ...
        if hasattr(rag_system, 'retriever') and hasattr(rag_system.retriever, "aget_relevant_documents"):
            rag_chain_accessible = hasattr(rag_system, 'rag_chain') and \
                                   hasattr(rag_system.rag_chain, 'steps') and \
                                   len(rag_system.rag_chain.steps) > 0 and \
                                   hasattr(rag_system.rag_chain.steps[0], "invoke")
