...
class ChatResponseSource(BaseModel):
    document_id: str = Field(description="Unique identifier for the source document (e.g., from metadata).")
    source_name: str = Field(description="Display name of the source (e.g., filename, lecture title).")
    page_number: Optional[int] = Field(None, description="Page number within the document, if applicable.")
    content_snippet: Optional[str] = Field(None, description="A small snippet of the relevant content.")

class ChatResponse(BaseModel):
    answer: str = Field(description="The AI-generated answer to the query.")
    sources: List[ChatResponseSource] = Field(default_factory=list, description="List of sources used to generate the answer.")
...
class ChatService:
    def __init__(self, rag_system_instance: Optional[RAGSystem] = None):
        ...
        """
        try:
            self.rag_system: RAGSystem = rag_system_instance or get_rag_system_instance()
            ...
            raise RuntimeError(f"ChatService could not be initialized: RAG system unavailable. {e}")
        except Exception as e:
            logger.error(f"An unexpected error occurred during ChatService initialization: {e}", exc_info=True)
            raise

    async def process_chat_query(self, query_text: str, user_id: Optional[str] = None, group_id: Optional[str] = None) -> ChatResponse:
        """
        ...
        """
        logger.info(f"Processing chat query: '{query_text}' for user '{user_id}', group '{group_id}'")

        if not self.rag_system:
            # This check is somewhat redundant due to __init__ raising error, but good for safety.
            logger.error("RAG system is not available in ChatService. This should not happen if initialization was successful.")
        ...
        try:
            ...
            if self.rag_system.retriever:
                ...
                source_documents = await self.rag_system.retriever.aget_relevant_documents(
                    query_text,
                    **retriever_kwargs
                )
            ...
            for doc in source_documents:
                ...
                retrieved_sources_formatted.append(
                    ChatResponseSource(
                        ...
                        source_name=str(doc.metadata.get("source", doc.metadata.get("filename", "Unknown Source"))),
                        page_number=doc.metadata.get("page_number", None),
                        content_snippet=doc.page_content[:200] + "..." if doc.page_content else "" # Example snippet
                    )
                )
            
            response = ChatResponse(answer=answer_text, sources=retrieved_sources_formatted)
            logger.info(f"Generated ChatResponse for query '{query_text}'. Answer: '{answer_text[:100]}...'")
            return response

        except Exception as e:
            logger.error(f"Error processing chat query '{query_text}': {e}", exc_info=True)
...
def get_chat_service_instance() -> ChatService:
    ...
    if chat_service_instance is None:
        ...
        try:
            ...
            logger.fatal(f"CRITICAL: Failed to initialize ChatService instance: {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    if env_path:
        ...
        logger.warning("No .env file found. Relying on environment variables or defaults for OPENAI_API_KEY.")
    ...
    class MockChatServiceRetriever(BaseRetriever):
        def _get_relevant_documents(self, query: str, *, run_manager=None, **kwargs) -> List[Document]:
            logger.info(f"[MockChatServiceRetriever SYNC] Getting documents for: {query}, kwargs: {kwargs}")
            docs = []
            if "python" in query.lower():
                docs.append(Document(page_content="Python is a versatile programming language.", metadata={"doc_id": "py001", "source": "python_intro.pdf", "page_number": 1}))
            if " AVL tree" in query.lower(): # From rag_pipeline test
                 docs.append(Document(page_content="AVL trees use rotations.", metadata={"doc_id": "avl002", "source": "datastructures.txt"}))
            
            if kwargs.get('metadata') and kwargs['metadata'].get('group_id') == 'group123':
                 docs.append(Document(page_content="This document is specific to group123.", metadata={"doc_id": "grp001", "source": "group_doc.pdf"}))
        ...
        async def _aget_relevant_documents(self, query: str, *, run_manager=None, **kwargs) -> List[Document]:
            logger.info(f"[MockChatServiceRetriever ASYNC] Getting documents for: {query}, kwargs: {kwargs}")
    ...
    async def main_chat_test():
        ...
        with patch('app.core.rag_pipeline.get_retriever', return_value=mock_retriever_instance) as mock_get_retriever_func:

            try:
                rag_system_for_test = RAGSystem() 

            except Exception as e:
                logger.error(f"Failed to initialize RAGSystem for ChatService test: {e}. Check OPENAI_API_KEY and imports.")
                logger.error("If OPENAI_API_KEY is missing, LLMProvider in llm.py or EmbeddingProvider in embeddings.py will raise ValueError.")
                return
            ...
            try:
                ...
                response_1 = await chat_svc.process_chat_query(test_query_1, user_id="test_user_01")
                ...
                assert "Python" in response_1.answer or "versatile" in response_1.answer # depends on LLM
                ...
                response_2 = await chat_svc.process_chat_query(test_query_2, user_id="test_user_02", group_id="group123")
                ...
                assert any("grp001" in s.document_id for s in response_2.sources), "Group-specific document not found"

                test_query_3 = "What is the weather today?" 
                logger.info(f"\n[Test 3] Query: \"{test_query_3}\" (no specific context in mock)")
                response_3 = await chat_svc.process_chat_query(test_query_3, user_id="test_user_03")
                ...
                assert len(response_3.sources) == 0 or not any(s.document_id for s in response_3.sources if s.document_id not in ["unknown"])
    ...
    if __name__ == '__main__': # Ensure this check again for clarity if script is run directly
        try:
