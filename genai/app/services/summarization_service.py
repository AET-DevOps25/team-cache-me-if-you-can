...
class SummarizationService:
    ...
    async def summarize_document(self, source_filename: str, tenant: str) -> str:
        """
        Generates a summary for a given document by its source filename.
        """
        logger.info(f"Starting summarization for document: {source_filename} in tenant {tenant}")

        # 1. Retrieve all document chunks
        document_chunks: List[Document] = await get_all_documents_for_source(source_filename, tenant)

        if not document_chunks:
            logger.warning(f"No document chunks found for '{source_filename}' in tenant {tenant}. Cannot generate summary.")
            return "Could not find the document to summarize. Please ensure it has been uploaded and processed."

        logger.info(f"Found {len(document_chunks)} chunks for summarization in tenant {tenant}.")
        ...
        try:
            ...
            logger.info(f"Successfully generated summary for {source_filename} in tenant {tenant}.")
            return result
        except Exception as e:
            logger.error(f"Error during summarization for {source_filename} in tenant {tenant}: {e}", exc_info=True)
