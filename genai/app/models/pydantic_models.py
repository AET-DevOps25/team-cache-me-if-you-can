...
class DocumentUploadResponse(BaseModel):
    ...
    document_id: str = Field(
        ..., description="Unique identifier for the uploaded and processing document."
    )
    filename: str = Field(
...
class DocumentStatus(BaseModel):
    status: str = Field(
        ...,
        description="Processing status of the document (e.g., PENDING, PROCESSING, COMPLETED, FAILED).",
    )
...
class DocumentStatusResponse(BaseModel):
    ...
    status_info: DocumentStatus = Field(
        ..., description="Detailed status of the document."
    )
