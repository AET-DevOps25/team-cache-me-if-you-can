...
from app.models.schemas import QueryRequest, QueryResponse, DocumentMetadata, SourceDocument
...
class MockRetriever:
    async def aget_relevant_documents(self, query: str):
        ...
        if "no_docs_query" in query:
            return []
        return [
            SourceDocument(page_content="Mocked relevant document content.", metadata=DocumentMetadata(source="mock_source.pdf", page_number=1)),
            SourceDocument(page_content="Another mocked relevant document.", metadata=DocumentMetadata(source="mock_source_2.pdf"))
        ]
...
def test_query_document_empty_question(chat_test_client: TestClient):
    response = chat_test_client.post(
        "/api/v1/chat/query",
        json={"question": "   "} # Empty or whitespace only
    )
